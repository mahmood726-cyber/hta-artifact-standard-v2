...
class ModelAveragingEngine {
    ...
    _fitWeibull(data) {
        ...
        for (let iter = 0; iter < NR_MAX_ITER; iter++) {
            ...
            if (Math.abs(det) > 1e-20) {
                ...
                const maxStep = Math.max(Math.abs(dLogShape), Math.abs(dLogScale));
                if (maxStep > 2) { const damping = 2 / maxStep; dLogShape *= damping; dLogScale *= damping; }
                logShape += dLogShape;
                logScale += dLogScale;
            } else {
    ...
    _fitLognormal(data) {
        ...
        const sigma2 = events.reduce((s, d) => s + (d.logT - mu) ** 2, 0) / events.length;
    ...
    _fitLoglogistic(data) {
        ...
        for (let iter = 0; iter < NR_MAX_ITER; iter++) {
            ...
            const H11 = (gAlphaP - gAlpha) / eps;
            const H12_a = (gBetaP - gBeta) / eps;
            ...
            const H12_b = (gAlphaP - gAlpha) / eps;
            const H22 = (gBetaP - gBeta) / eps;
            const H12 = 0.5 * (H12_a + H12_b);
            ...
            if (Math.abs(det) > 1e-20) {
                ...
                const maxStep = Math.max(Math.abs(dLogAlpha), Math.abs(dLogBeta));
                if (maxStep > 2) { const damping = 2 / maxStep; dLogAlpha *= damping; dLogBeta *= damping; }
                logAlpha += dLogAlpha;
                logBeta += dLogBeta;
            } else {
                // Fallback to gradient ascent
    ...
    _fitGamma(data) {
        ...
        for (let iter = 0; iter < NR_MAX_ITER; iter++) {
            ...
            const H11 = (gShapeP - gShape) / eps;
            const H12_a = (gRateP - gRate) / eps;
            ...
            gShapeP = gs_p * shapeP;
            gRateP = gr_p * rateP;
            const H12_b = (gShapeP - gShape) / eps;
            const H22 = (gRateP - gRate) / eps;
            const H12 = 0.5 * (H12_a + H12_b);
            ...
            if (Math.abs(det) > 1e-20) {
                ...
                const maxStep = Math.max(Math.abs(dLogShape), Math.abs(dLogRate));
                if (maxStep > 2) { const damping = 2 / maxStep; dLogShape *= damping; dLogRate *= damping; }
                logShape += dLogShape;
                logRate += dLogRate;
            } else {
                // Fallback to gradient ascent
    ...
    _fitGompertz(data) {
        ...
        for (let iter = 0; iter < NR_MAX_ITER; iter++) {
            ...
            const computeGrad = (b_p, eta_p) => {
                let gb = 0, ge = 0;
                for (const d2 of data) {
                    if (d2.time <= 0) continue;
                    const et2 = Math.min(Math.max(eta_p * d2.time, -500), 500);
                    const expET2 = Math.exp(et2);
            ...
            const [gb_pb, ge_pb] = computeGrad(b + eps_b, eta);
            const H11 = (gb_pb - g_b) / eps_b;
            const H12_a = (ge_pb - g_eta) / eps_b;

            const [gb_pe, ge_pe] = computeGrad(b, eta + eps_eta);
            const H12_b2 = (gb_pe - g_b) / eps_eta;
            const H22 = (ge_pe - g_eta) / eps_eta;
            const H12 = 0.5 * (H12_a + H12_b2);

            const det = H11 * H22 - H12 * H12;
            if (Math.abs(det) > 1e-20) {
                let dB = -(H22 * g_b - H12 * g_eta) / det;
                let dEta = -(-H12 * g_b + H11 * g_eta) / det;
                const maxStep = Math.max(Math.abs(dB), Math.abs(dEta));
                if (maxStep > 2) { const damping = 2 / maxStep; dB *= damping; dEta *= damping; }
                b += dB;
                eta += dEta;
            } else {
                // Fallback to gradient ascent
    ...
    survivalPrediction(fittedModels, times) {
        ...
        const totalWeight = survCurves.reduce((s, c) => s + c.weight, 0);
    ...
    _survivalFunction(dist, params, t) {
        ...
        switch (dist) {
            ...
            case 'gompertz': {
                ...
                const etaT = Math.min(Math.max(params.eta * t, -500), 500);
                let cumHaz;
